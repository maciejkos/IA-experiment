...
var Experiment = {
	...
	logUrl: '/pulse', // the url which receives periodic experimental data via POST  
	nextUrl: '/prev', // the url after the experiment concludes
	...
	experimentDuration: 300, //  (should be 1300) in seconds
	questionMinRandomNumber: 1000000, // (should be 10) the minimum value for a random number in a random number flash card question
	questionMaxRandomNumber: 9999999, // (should be 99) the maximum value for a random number in a random number flash card question
	
	logTimerDuration: 60, // (should be 10) in seconds.  The duration of time until the experiment data is periodically logged
   ...
   afterAnswerTimerDuration: 20, // (should be 20) in seconds. the amount of time the user must wait after they have answered a question.
	...
	radioButtonQuestionConfigs: [
		{
			'form_input_name': 'q1', 
			'question_text':'Wskaż na poniższej skali, jak byś się czuł, gdybyś się dowiedział/a, że na pewno utracisz 90% pieniędzy? <br>', 
			'answer_option_texts': ['bardzo niezadowolony', 'trochę niezadowolony', 'ani niezadowolony, ani zadowolony', 'zadowolony', 'bardzo zadowolony'],
			'offer_answer_timer_duration': 0, // seconds (should be 20 )
			'after_answer_timer_duration': 0, // seconds (should be 0)
		},
		{
			'form_input_name': 'q2', 
			'question_text':'Czy chcesz dowiedzieć się, jakie jest Twoje prawdopodbieństwo utraty pieniędzy pod koniec eksperymentu?<br> Wyświetli się ono po wpisaniu bieżącego ciągu cyfr. <br> Na podjęcie decyzji masz 30 sekund.',
			'answer_option_texts': ['tak', 'nie'],
			'offer_answer_timer_duration': 30,  // seconds (should be 20 )
			'after_answer_timer_duration': 0,  // seconds (should be 0 )
		},
	],
	...
	endExperiment: function() {
	   ...
	   console.log('Thank you for participating in the experiment!');
...
var QuestionFactory = {
	...
	createQuestions: function() {
		...
		for (var i = 0; i < primerQuestionCount; i++) {
			...
			textQuestion = QuestionFactory.createTextQuestionForPrimerQuestion('Wpisz zapamiętaną liczbę', primerQuestion);
...
RadioButtonQuestion.prototype = {
	...
	createForm: function() {
		...
		dHtml += '</fieldset>';
		qHtml += dHtml;
		qHtml += '<input type="submit" value="Dalej"/></form>';
		return qHtml;
	},
	...
	addFormSubmitLogic: function() {
		...
		$('#question_form').on('submit', function(e) {
			   ...
			   if (self.data['after_answer_timer_duration'] > 0) {
				   $('#question_content').html('<p>' + self.data['after_answer_timer_duration'] + ' sekund przerwy.</p>');
			   }
	...
	addFormSubmitLogic: function() {
		...
		$('#question_form').on('submit', function(e) {
			   ...
			   if (self.data['offered_answer'] == self.data['correct_answer']) {
				   ...
				   $('#question_content').html('<p>Prawidłowa odpowiedź</p><p>' + self.data['after_answer_timer_duration'] + ' sekund przerwy.</p>');
			   } else {
				   $('#question_content').html('<p>Nieprawidłowa odpowiedź</p><p>' + self.data['after_answer_timer_duration'] + ' sekund przerwy.</p>');
			   }
...
$(function() {
	...
	console.log('Welcome ' + $.cookie('username') + '.  Click OK to begin the experiment.');
